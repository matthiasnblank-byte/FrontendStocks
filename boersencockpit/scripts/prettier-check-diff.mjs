...
const resolveBase = () => {
  if (process.env.PRETTIER_DIFF_BASE) {
    return process.env.PRETTIER_DIFF_BASE;
  }
...
const files = output
  ...
  .filter((file) => file.match(/\.(ts|html|css|json|md)$/i));

if (files.length === 0) {
  console.log('No files to check with Prettier.');
  process.exit(0);
}
